    ...
    <div className="flex min-h-screen bg-background text-foreground transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className={`fixed inset-0 z-40 md:hidden flex transition-all duration-300 ${
          ...
          <div className={`relative flex flex-col w-64 max-w-xs bg-[#f4f4f5] dark:bg-zinc-950 border-r border-border p-5 z-50 transition-transform duration-300 ease-in-out ${
            mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
          }`}>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <div className="h-7 w-7 rounded-lg bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center font-black text-white text-xs shadow-sm">
                  FM
                </div>
                <span className="font-bold text-sm text-foreground">FirmanMan</span>
              </div>
              <button
                onClick={() => setMobileMenuOpen(false)}
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className={`flex items-center gap-2.5 px-3 py-2 rounded-md text-sm font-semibold transition-all ${
                      isActive
                        ? 'bg-zinc-200/50 dark:bg-zinc-800/60 text-foreground'
                        : 'text-muted-foreground hover:text-foreground hover:bg-muted/40'
                    }`}
